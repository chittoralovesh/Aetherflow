...
async function callLLM(prompt: string, apiKey?: string): Promise<string> {
  // Artificial delay for LLM simulation or real API call
  ...
  if (key) {
    try {
      ...
      console.error('Failed to call Gemini API in Nhost Function, falling back to stub:', e);
    }
  }
...
export async function executeWorkflowRun(
  ...
  try {
    ...
    for (let i = startIndex; i < steps.length; i++) {
      ...
      // Check if we already have a step run for this step
        ...
        try {
          if (step.type === 'llm_call') {
            ...
            if (!isMatch) {
              ...
              // Insert dummy 'completed' entries for remaining steps to finish gracefully
    ...
    // Increment organization usage quota
    await query(
      'UPDATE organizations SET calls_used = calls_used + 1 WHERE id = $1',
      [run.org_id]
    );
